...
class GeneratePayslipHandler
{
    ...
    {
        $employee = $this->entityManager->getRepository(Employee::class)->find($message->getEmployeeId());
    ...
    private function generatePdf(Employee $employee, PayrollPeriod $period, array $accruals, array $deductions): string
    {
        ...
        $pdf->Cell(0, 6, 'Period: '.$period->getStartDate()->format('Y-m-d').' - '.$period->getEndDate()->format('Y-m-d'), 0, 1);
        $pdf->Ln(10);
